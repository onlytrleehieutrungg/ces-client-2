...
export default function EmployeeOrderCustom({ employeeId }: Props) {
  const {
    dense,
    page,
    order,
  ...
  const { data: orderSup, isLoading: isOrderLoading } = useOrder({ params })

  useEffect(() => {
    if (employeeId) {
      setParams({ ...params, EmployeeId: employeeId })
    }
  }, [employeeId, params])
